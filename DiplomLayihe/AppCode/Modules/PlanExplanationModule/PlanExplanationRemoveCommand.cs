...
{
    public class PlanExplanationRemoveCommand : IJsonRequest
    {
        public int Id { get; set; }

        public class PlanExplanationRemoveCommandHandler : IJsonRequestHandler<PlanExplanationRemoveCommand>
        {
            readonly DiplomDbContext db;
            private readonly IActionContextAccessor ctx;

            public PlanExplanationRemoveCommandHandler(DiplomDbContext db,
                IActionContextAccessor ctx)
            {
                this.db = db;
                this.ctx = ctx;
            }
            public async Task<CommandJsonResponse> Handle(PlanExplanationRemoveCommand request, CancellationToken cancellationToken)
            {
