...
{
    public class CourseCategoriesRemoveCommand : IJsonRequest
    {
        public int Id { get; set; }

        public class CourseCategoriesRemoveCommandHandler : IJsonRequestHandler<CourseCategoriesRemoveCommand>
        {
            readonly DiplomDbContext db;
            private readonly IActionContextAccessor ctx;

            public CourseCategoriesRemoveCommandHandler(DiplomDbContext db,
                IActionContextAccessor ctx)
            {
                this.db = db;
                this.ctx = ctx;
            }
            public async Task<CommandJsonResponse> Handle(CourseCategoriesRemoveCommand request, CancellationToken cancellationToken)
            {
