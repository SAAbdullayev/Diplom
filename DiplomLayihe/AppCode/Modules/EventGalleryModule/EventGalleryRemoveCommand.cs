...
{
    public class EventGalleryRemoveCommand : IJsonRequest
    {
        public int Id { get; set; }

        public class EventGalleryRemoveCommandHandler : IJsonRequestHandler<EventGalleryRemoveCommand>
        {
            readonly DiplomDbContext db;
            private readonly IActionContextAccessor ctx;

            public EventGalleryRemoveCommandHandler(DiplomDbContext db,
                IActionContextAccessor ctx)
            {
                this.db = db;
                this.ctx = ctx;
            }
            public async Task<CommandJsonResponse> Handle(EventGalleryRemoveCommand request, CancellationToken cancellationToken)
            {
