...
{
    public class PlanAndPricingRemoveCommand : IJsonRequest
    {
        public int Id { get; set; }


        public class PlanAndPricingRemoveCommandHandler : IJsonRequestHandler<PlanAndPricingRemoveCommand>
        {
            readonly DiplomDbContext db;
            private readonly IActionContextAccessor ctx;

            public PlanAndPricingRemoveCommandHandler(DiplomDbContext db,
                IActionContextAccessor ctx)
            {
                this.db = db;
                this.ctx = ctx;
            }
            public async Task<CommandJsonResponse> Handle(PlanAndPricingRemoveCommand request, CancellationToken cancellationToken)
            {
